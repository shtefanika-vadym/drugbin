...
import { Container, FilterBox, Filters, HeaderActions, HeaderRow, TableBody, TableHeader } from './list.styled'
...
const GRID = 'minmax(0, 1.4fr) minmax(0, 0.6fr) minmax(0, 1fr) minmax(0, 2fr) minmax(0, 1.6fr) minmax(0, 1fr)'
...
export const Classifications = () => {
  ...
  const { items, total, totalPages, isLoading, refresh } = useClassifications(filters, page, pageSize)
  ...
  const renderRows = () => {
    ...
    return items.map((c) => (
      <TableRow key={c.imageId} onClick={() => navigate(`/admin/clasificari/${c.imageId}`)}>
  ...
  return (
    <Container>
      <HeaderRow>
        <Text variant='titleH4'>Clasificări</Text>
        <HeaderActions>
          <RefreshButton onRefresh={refresh} />
          <Button onClick={() => toggleSimulate(true)}>Simulează</Button>
        </HeaderActions>
      </HeaderRow>

      <Filters>
        <FilterBox>
          <Select label='Tier' value={filters.tier ?? ''} onChange={(e) => setFilter('tier', e.target.value)}>
